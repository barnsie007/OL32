...
# This configuration was migrated on 5/31/22 at 10:35:30 PM from the following location: /opt/IBM/WebSphere/AppServer/profiles/AppSrv03
...
# ============================================================
ghettoes1Node04_db2_password_1=''
ghettoes1Node04_db2_user_1=''
# ============================================================

print 'Starting Creating JVM Properties'
# Properties are migrated from server ghettoes1Node04/server1.

print 'Starting Creating Authentication Alias'
GlobalSecurityVar=AdminConfig.getid('/Cell:' + AdminControl.getCell() + '/' + 'Security:/')
AdminConfig.create('JAASAuthData', GlobalSecurityVar, [['alias', 'ghettoes1Node04/db2'], ['description', ''], ['password', ghettoes1Node04_db2_password_1], ['userId', ghettoes1Node04_db2_user_1]])
...
AdminConfigVar_4=AdminConfig.create('JDBCProvider', Node, [['classpath', '/root/automation/driver/db2jcc.jar'], ['description', 'Two-phase commit DB2 JCC provider that supports JDBC 3.0. Data sources that use this provider support the use of XA to perform 2-phase commit processing. Use of driver type 2 on the application server for z/OS is not supported for data sources created under this provider.'], ['implementationClassName', 'com.ibm.db2.jcc.DB2XADataSource'], ['name', 'DB2_Universal_JDBC_Driver_Provider_(XA)'], ['providerType', 'DB2 Universal JDBC Driver Provider (XA)'], ['xa', 'true']])
AdminConfigVar_5=AdminTask.createDatasource(AdminConfigVar_4, ["-name", "DB2 Universal JDBC Driver XA DataSource", "-jndiName", "jndi/db21", "-dataStoreHelperClassName", "com.ibm.websphere.rsadapter.DB2UniversalDataStoreHelper", "-componentManagedAuthenticationAlias", "ghettoes1Node04/db2", "-xaRecoveryAuthAlias", "ghettoes1Node04/db2", "-configureResourceProperties", "[[databaseName java.lang.String testdb] [driverType java.lang.Integer 4] [serverName java.lang.String 9.46.76.40] [portNumber java.lang.Integer 50000] ]"])
...
AdminConfig.create('J2EEResourceProperty', AdminConfigVar_6, [['name', 'name'], ['type', 'java.lang.String'], ['value', 'DB2 Universal JDBC Driver XA DataSource']])
...
AdminConfig.create('J2EEResourceProperty', AdminConfigVar_6, [['name', 'retrieveMessagesFromServerOnGetMessage'], ['type', 'java.lang.Boolean'], ['value', 'false']])
